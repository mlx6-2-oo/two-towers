...
def train():
    ...
    val_triple_dataloader = DataLoader(val_triple_dataset, batch_size=batch_size, shuffle=True, collate_fn=triple_dataset.collate_fn, drop_last=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (query_batch, document_batch, neg_document_batch) in enumerate(train_loop):
            ...
            # Compute loss
            loss, unclamped_loss, relevant_distance, irrelevant_distance = model.compute_loss(query_batch, document_batch, neg_document_batch)
            ...
            wandb.log({"batch_loss": loss.item(), "batch": batch_idx + epoch * len(triple_dataloader), "unclamped_loss": unclamped_loss.item(), "relevant_distance": relevant_distance.item(), "irrelevant_distance": irrelevant_distance.item()})
            ...
            if batch_idx % 100 == 0:
                with torch.no_grad():
                    ...
                    val_loss, unclamped_loss, relevant_distance, irrelevant_distance = model.compute_loss(val_query_batch, val_documents_batch, val_neg_documents_batch)
                    wandb.log({"batch_val_loss": val_loss.item(), "batch_val_unclamped_loss": unclamped_loss.item(), "batch_val_relevant_distance": relevant_distance.item(), "batch_val_irrelevant_distance": irrelevant_distance.item()})
